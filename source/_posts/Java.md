...
异常：程序运行时不正常情况。也是 java 对于不正常情况进行抽象后的体现，
...
方法 1: 将类声明为 Thread 的子类，子类重写 run 方法。run 方法存储线程要运行的代码。
...
线程结束：stop 已弃用，只有一种，run 结束可以结束线程。开启多线程运行，运行代码通常是循环结构。只要控制住循环，就可以 让 run 方法结束，也就是线程结束。
