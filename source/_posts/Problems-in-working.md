...
# 关于dialog 弹窗的几点思考
1. dialog关闭 是否还需要 弹窗组件
2. 由子组件通过emit事件触发父组件通过v-if 销毁整个组件。还是父组件通过$refs控制子组件中dialog的显隐，第3点解释了两种实现方式不同的原理。
...
# 递归性质
当一个复杂问题可以分解成诺干个子问题，并且子问题的结构和原问题的结构相同，那么就可以使用递归去解决。
使用递归要格外的关注 程序的出口条件的设置。