...
# JS 实现防抖和节流 
## 防抖debounce
防抖就是 在短时间内大量触发同一事件，只会执行一次事件函数。
在第一次触发事件时，不立即执行函数，而是给出一个期限值，在期限值内没有再次触发事件，那么就执行函数。在期限值内再次触发事件，取消当前的计时，重新开始计时。
```js
// 简单实现
...
```
## 节流throttle
节流：在短时间内大量触发同一事件，那么在函数执行一次后，该函数在指定的时间期限内不再工作，直到过了这段时间才重新生效。
```js
// 简单实现
