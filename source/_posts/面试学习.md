...
# Vue学习
## Vue 核心思想简介
Vue 核心思想：1.数据驱动应用 2.MVVM模式(Model, View, 。ViewModel)
...
```
### 组件缓存
重新创建动态组件的行为通常是非常有用的，但是有时候需要某个组件第一次被创建的时候状态就被缓存下来，再次切换到这个组件的时候，会以之前的状态显示。 利用keepalive做组件缓存，主要用于保留组件状态或避免重新渲染，提高执行效率。
```
// 设置include, max属性，include 表示要缓存的组件
<keep-alive :include="['admin', 'about']" :max="10">
    <router-view></router-view>
</keep-alive>
<!-- 逗号分隔字符串 -->
...
```
include 和 exclude 允许组件有条件地缓存。二者都可以用逗号分隔字符串、正则表达式或一个数组来表示。组件需要设置name选项，匹配首先检查组件自身的 name 选项，如果 name 选项不可用，则匹配它的局部注册名称 (父组件 components 选项的键值)。匿名组件不能被匹配。

max表示最多可以缓存多少组件实例。一旦这个数字达到了，在新实例被创建之前，已缓存组件中最久没有被访问的实例会被销毁掉。

keep-alive组件的两个特别的生命周期钩子函数： activated，deactivated。它们会在keep-alive内部嵌套的所有组件中触发。

### 状态管理 Vuex
Vuex是一个专为Vue应用程序开发的状态管理模式。它采用集中式存储管理应用的所有组件的状态，并以相应的规则保证状态以可预测的方式发生变化。
Vuex 中的数据的状态只能通过mutations来更改状态，如果直接在state上操作会报错，vuex 与 vue是紧耦合的，vuex利用了数据的响应式，来实现state变更时，触发视图更改。
#### 安装
```
vue add vuex
```
#### 使用
mutations是响应化的，只能是同步方法。
异步方法必须放在actions中。
```
import Vue from 'vue'
...
export default new Vuex.Store({
    ...
    // 派生状态，getter类似computed，只有当响应数据改变时，才会改变。
    getters： {
        welcome(state) {
    ...
    actions: {
        // 异步操作在actions中进行，要修改state也只能通过mutations
        // 第一个参数是vuex传递的上下文： {commit, dispatch, state}
...
辅助函数的结果是一个对象，使用辅助函数后都可以通过this来引用。
```
// 从vuex中引入mapState
...
export default {
    computed: {
        // 混入State的三种方式
        ...mapState(['login', 'logout']),
        ...mapState('moduleName', ['login', 'logout']), // 使用命名空间的方式
        ...mapState({ log: 'login' })                   // 对象方式，重命名
        ...mapState({                                   // 复杂的形式
            count: state => state.count,
            countAlias: 'count',
            countPlusLocalState (state) {               // 使用this关键字，不能使箭头函数。
        ...
        // 混入Getters的三种方式
        ...mapGetters(['xxx', 'ooo']),
        ...mapGetters('moduleName', ['xxx', 'ooo']), // 使用命名空间的方式
        ...mapGetters({ log: 'login' })              // 对象方式，重命名
    ...
    methods: {
        ...
        ...mapActions('user', ['login', 'logout']), // 使用命名空间的方式
        ...mapActions({add: 'increment'})           // 对象方式，重命名
        

        // 混入Mutations 三种方式
        ...mapMutations('xxx', ['increment', 'incrementBy']), 
        ...mapMutations(['increment', 'incrementBy']),        // 使用命名空间的方式
        ...mapMutations({add: 'increment'})                   // 对象方式，重命名
        
    }
}
```
##### 严格模式
严格模式下，无论何时发生了状态变更且不是由mutation函数引起的，将会抛出错误。这能保证所有的状态变更都能被调试工具跟踪到。
```
...
```
##### Vuex插件
Vuex的store接受plugins选项，这个选项暴露出每次mutation的钩子。Vue插件就是一个函数，接受store作为唯一的参数。
使用场景：实现用户登陆状态持久化。
```
// src/store/plugins/persist
...
export default store => {
    ...
    //  如果用户相关状态发生变化，执行回调函数，自动存入localStorage
...
// 注册插件, 插件在store初始化后调用
import myPlugin from './store/plugins/persist.js'
const store = new Vuex.Store({
    plugins: [myPlugin]
})
```
#### 路由懒加载
路由组件的懒加载能把不同路由对应的组件分割成不同的代码，然后当路由被访问的时候才加载对应的组件，这样就更加高效了。
```
{
    path: '/about',
    name: 'about',
...
# To do list
## 事件派发方式的祖先和孙子组件之间的通信方式-混入dispatch方法

# Vue源码学习1
Vue响应式注意点：
1. 没有在data中声明的变量，是没有响应式的。
2. 对于数组的处理只有7个变异方法改数组后，数组还是响应化的，对于delete数组或者是用索引改变数组，数组都是无法响应的。
# Vue源码学习2
1. Vue中如何处理双向绑定(v-model)。
2. Vue中如何处理事件。
# Vue源码component组件化机制
# Vue源码 patch
# Vue v-for v-if v-model mixin Vue.extend Vue.component 
